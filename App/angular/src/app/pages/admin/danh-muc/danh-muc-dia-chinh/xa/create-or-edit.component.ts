...
import {DanhMucXaServiceProxy} from '@shared/service-proxies/service-proxies';
...
export class CreateOrEditXaComponent extends ModalComponentBase implements OnInit {
    @Input() dataItem: any;
    ...
    save(): void {
        if (this.rfDataModal.invalid) {
            ...
            this.saving = true;
            this._dataService.insertOrUpdate(this.rfDataModal.value)
                .pipe(finalize(() => {
                    this.saving = false;
                }))
                .subscribe(result => {
