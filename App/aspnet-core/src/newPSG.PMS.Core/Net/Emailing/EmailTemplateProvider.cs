...
namespace newPSG.PMS.Net.Emailing
{
    public class EmailTemplateProvider : IEmailTemplateProvider, ISingletonDependency
    {
        ...
        public string GetDefaultTemplate(int? tenantId)
        {
            ...
            {
                using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("newPSG.PMS.Net.Emailing.EmailTemplates.default.html"))
                {
        ...
        private string GetTenantLogoUrl(int? tenantId)
        {
            ...
            return _webUrlService.GetServerRootAddress().EnsureEndsWith('/') + "TenantCustomization/GetImgNutifood";
        }
    }
}