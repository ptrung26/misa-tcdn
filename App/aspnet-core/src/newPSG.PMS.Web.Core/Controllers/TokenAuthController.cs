...
using newPSG.PMS.Authentication.TwoFactor.Google;
...
namespace newPSG.PMS.Web.Controllers
{
    ...
    public class TokenAuthController : PMSControllerBase
    {
        ...
        [HttpPost]
        public async Task<ReturnReponse<AuthenticateResultModel>> AuthenticateNguoiDung([FromBody] AuthenticateNguoiDungInput input)
        {
            var res = new ReturnReponse<AuthenticateResultModel>();
            try
            {
                var loginResult = await GetLoginResultAsync(
                  input.AuthenModal.UserNameOrEmailAddress,
                  input.AuthenModal.Password,
                  GetTenancyNameOrNull()
                );
                ...
                try
                {
                    ...
                    {
                        input.FireBaseToken.UserId = loginResult.User.Id;
                        await _fireBaseService.CreateOrEdit(input.FireBaseToken);
                    }
                }
